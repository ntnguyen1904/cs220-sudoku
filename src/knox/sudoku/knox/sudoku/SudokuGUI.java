 ...
 * 
 * It extends JFrame, which means that it is a subclass of JFrame. The JFrame is
 * the main class, and owns the JMenuBar, which is the menu bar at the top of
 * the screen with the File and Help and other menus.
 * 
 * 
 * One of the most important instance variables is a JCanvas, which is kind of
 * like the canvas that we will paint all of the grid squared onto.
 * 
 * @author jaimespacco
 *
 */
public class SudokuGUI extends JFrame {
	...
	private static Font FONT = new Font("Verdana", Font.BOLD, 30);
	...
	private class ButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// turn off hints as soon as you click any grid square
	...
	/**
	 * This is a private helper method that updates the GUI/view to match any
	...
	private void completeColumn(int row, int col) {
		if (sudoku.completeCol(row, col)) {
			for (int r = 0; r < 9; r++) {
				buttons[r][col].setBackground(Color.YELLOW);
				}
			}
		repaint();
	}
	
	private void completeRow(int row, int col) {
		if (sudoku.completeRow(row, col)) {
			for (int c = 0; c < 9; c ++) {
				buttons[row][c].setBackground(Color.YELLOW);
				}
			}
		repaint();
	}
	...
	private void createMenuBar() {
		...
		addToMenu(file, "Save", new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					JOptionPane.showMessageDialog(null, "Save game to file " +  selectedFile.getAbsolutePath());
				}
		...
		addToMenu(file, "Load", new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// HINT: check out JFileChooser
				// https://docs.oracle.com/javase/tutorial/uiswing/components/filechooser.html
				JOptionPane.showMessageDialog(null, "TODO: load a saved game from a file\n"
						+ "HINT: Check the Util.java class for helpful methods\n" + "HINT: Check out JFileChooser");
				update();
			}
		});
